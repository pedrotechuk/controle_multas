...
mount(function () {
    ...
    $allColumns = Schema::getColumnListing('multas');
    ...
    $this->multasData = DB::table('multas')
        ->select($this->columns)
        ->get();
});
...
$export = function () {
    $fileName = 'controle_multas_' . date('dmY') . '.csv';
...
<div class="mb-6">
    <div class="flex justify-between items-center bg-[#2f4d93] text-white p-4 rounded-t-lg shadow-lg mb-2">
        <h1 class="font-semibold text-lg">Exportar Base de Multas</h1>
    </div>

    <div class="flex flex-col bg-white rounded-lg p-6 border border-blue-200 shadow-md hover:shadow-lg transition duration-300">
        <h2 class="font-semibold text-[#2f4d93] mb-4">Exportar Multas</h2>
        <p class="text-gray-700 mb-4">Exportar a lista de todos as multas com as colunas disponíveis no momento.</p>
